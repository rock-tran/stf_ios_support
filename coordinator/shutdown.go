...
func cleanup_procs(config *Config) {
    ...
    for _, proc := range procs {
        ...
        if cmd[0] == "/Users/mobilehealth/.nvm/versions/node/v14.21.3/bin/node" && cmd[3] == "device-ios" {
            pid := proc.PID()
        ...
        if cmd[0] == "/Users/mobilehealth/.nvm/versions/node/v14.21.3/bin/node" && cmd[3] == "provider" {
            pid := proc.PID()
